...
public class X12Group {
    ...
    /** Creates a new instance of X12Group */
    public X12Group() throws JDOMException, IOException {
        initialize();
    }
    
    public X12Group(X12File file) throws JDOMException, IOException{
        initialize(file);
    }
    ...
    void initialize()throws JDOMException, IOException{
        SAXBuilder saxBuilder = new SAXBuilder();
        Document interchangeDoc = saxBuilder.build(System.getProperty("application.templates") + System.getProperty("file.separator") + "x12" + System.getProperty("file.separator") + "group.xml");
        Element root = interchangeDoc.getRootElement();
        List l = root.getChildren("segment");
        Iterator it = l.iterator();
        while(it.hasNext()){
